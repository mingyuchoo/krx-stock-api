...
export default {
  ...
  async getData(code: string) {
    ...
    // @ts-ignore
    const data = xmlJs.xml2js(body.toString(), { compact: true }).stockprice;
    data.TBL_DailyStock = data.TBL_DailyStock.DailyStock.map(
      (value) => value._attributes
    );
    data.TBL_StockInfo = data.TBL_StockInfo._attributes;
    data.TBL_Hoga = data.TBL_Hoga._attributes;
    data.TBL_TimeConclude = data.TBL_TimeConclude.TBL_TimeConclude.map(
      (value) => value._attributes
    );
    data.stockInfo = data.stockInfo._attributes;
    data.TBL_AskPrice = data.TBL_AskPrice.AskPrice.map(
      (value) => value._attributes
    );
    return data;
  },
